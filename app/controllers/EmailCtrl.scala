...
object EmailCtrl extends Controller with ProvidesCtx with Security {
  ...
  def editOrgEmail (oid: Long, id: Long) = isAuthenticated { username =>
    implicit request =>
      val orgOp = Organization.load(oid)
      if (orgOp.isDefined) {
  ...
  def showPersonEmail (pid: Long) = isAuthenticated { username =>
    implicit request =>
      val persOp = Person.load(pid)
      if (persOp.isDefined) {
        val persEmails = Email.getPersonEmails(persOp.get)
        if (persEmails.isSuccess) {
          Ok(views.html.email(persOp.get, persEmails.toOption.get))
        } else {
          Logger.error(s"Failed to load email addresses for person '${persOp.get.fullname}'")
    ...
    implicit request =>
      emailForm.bindFromRequest.fold(
        ...
        email => {
          val persOp = Person.load(pid)
          if (persOp.isDefined) {
            Logger.debug(s"Storing person email '${email._1.address}' for person '${persOp.get.fullname}'")
            val result = Email.savePersonEmail(persOp.get, email._1, email._2, email._3)
            if (result.isSuccess) {
